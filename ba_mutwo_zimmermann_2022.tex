...
Die anspruchsvolle Spezifikation \emph{mutwos}, besonders der generischen Strukturen, steht projektbezogenen Arbeiten (und den damit verbundenen Fristen) unfreundlich entgegen.

\smallskip

In der Version \texttt{0.17.1} des Pakets \texttt{mutwo.music} wird der Parameter \texttt{PlayingIndicator} definiert.
...
Um die Klasse \texttt{WoodwindFingering} zu initialisieren sind z.~B. die Argumente \texttt{cc}, \texttt{left\_hand} und \texttt{right\_hand} erforderlich.
Die Argumente erwarten jeweils einen Tupel, der mit Zeichenketten gefüllt ist.
Diese Form der Repräsentation von Griffen entspricht dem Befehl in \emph{Lilypond}. 
...
Die innere Repräsentationen \emph{mutwos} wäre dann so gestaltet, dass sie offen und neutral gegenüber unterschiedliche dritte Formate wie \emph{MusicXML} oder \emph{Lilypond} wäre.
Die Ursache der problematischen Situation ist eine Mischung aus einer projektspezifischen Notwendigkeit für eine bestimmte, noch abwesende Funktion und zugleich der projektspezifischen Einschränkung der Ressource Zeit.
...
% Ich habe vor allem die Software entwickelt, dh. Dinge die ich benutzt habe sind
% bereits einfach darstellbar (hier gilt: "battery-included"). Aber Dinge die mich nicht
% interessieren gibt es da noch nicht. Dh. wenn jemand sich für Ähnliches wie ich interessiert
% ist es für diese Person einfacher (geringere Einstiegshürde) mutwo zu benutzen.
%
% (oder es ist einfacher sachen damit zu machen die meiner ästhetik entsprechen)


% Wirklichkeit (Pragmatismus) vs. Theorie (Idealismus)
%
%  gegenwärtige implementierung:
%
%    1. notation_indicators und playing_indicators sind sehr von Lilypond geprägt.
%    2. Ereignisse können nur das Koordinatensystem Dauer verwenden.
%
% -> -> es geht immer um die arbeit an wirklichen projekten, weshalb manchmal wichtiger ist
%       einfach irgendwas fertig zu machen, das funktioniert.
...
% Hier ist die Idee ein Art minimales Gamelan Beispiel zu geben.
%
% Und zwar:
%
